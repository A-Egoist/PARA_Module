...
class PARA(nn.Module):
    def __init__(self, backbone, device, num_users, num_items, embedding_dim, n_layers, adjustment_coefficient, method_config, graph=None):
        super().__init__()
        ...
        self.item_adjustment_coefficient = adjustment_coefficient  # tensor
    ...
    def forward(self, user_indices, item_i_indices, item_j_indices=None):
    ...
    def predict(self, user_indices):
        ...
        prediction = F.softplus(torch.matmul(user_vector, item_vector.t())) * torch.sigmoid(self.item_adjustment_coefficient)
        return prediction